	...
	with (codecs.open('test.csv' , 'w',encoding ='utf-8',errors='ignore')) as fp:
		...
		while (i < 10):
			...
			for x in range(0 , len(content['statuses'])):
				...
				place = content['statuses'][x].get('place',None)
					
				if place :
					lat_lng3 = place.get('bounding_box',None)
					if lat_lng3:
						lat_lng2 = lat_lng3.get('coordinates', None)
						if lat_lng2:
							latA_str = str(lat_lng2[0][0][0])
							lngA_str = str(lat_lng2[0][0][1])
							latB_str = str(lat_lng2[0][1][0])
  							lngB_str = str(lat_lng2[0][1][1])
  							latC_str = str(lat_lng2[0][2][0])
 							lngC_str = str(lat_lng2[0][2][1])
  							latD_str = str(lat_lng2[0][3][0])
  							lngD_str = str(lat_lng2[0][3][1])
						else:
							latA_str = ''
							lngA_str = ''
							latB_str = ''
...
f.writerow (["tweet ID","user ID","tweet coordinates",
			...
			"place_coordinate_lat_A", "place_coordinate_lng_A",
								"place_coordinate_lat_B","place_coordinate_lng_B","place_coordinate_lat_C","place_coordinate_lng_C","place_coordinate_lat_D",
								"place_coordinate_lng_D",
